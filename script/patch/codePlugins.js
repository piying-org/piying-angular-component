  ...
  setup(build) {
    ...
    for (const name of ['text', 'title']) {
      build.onLoad({ filter: /.*/, namespace: `raw-code-${name}` }, (args) => {
        ...
        if (
          node_fs_1.default.existsSync(filePath) &&
          node_fs_1.default.lstatSync(filePath).isDirectory()
        ) {
        ...
        if (name === 'text') {
          ...
          console.log(build.initialOptions.absPaths);
          console.log(build.initialOptions.absWorkingDir);
          console.log(filePath);
          let baseDir = path.join(
            build.initialOptions.absWorkingDir,
            'src/app/piying/page/component',
          );
