...
@Component({
  ...
  imports: [AttributesDirective, NgTemplateOutlet, MatIcon, AttributesDirective],
})
export class FabNFCC {
  ...
  optionClass = input('btn btn-lg btn-circle');
  ...
  closeIcon = input<FabOption>({
    label: '✕',
    class: 'btn btn-circle btn-lg btn-error',
  });
  closeIcon$$ = computed(() => {
    return new FabItem(this.closeIcon());
  });
  mainIcon = input<FabOption>();
  mainIcon$$ = computed(() => {
    let item = this.mainIcon();
    return item ? new FabItem(item) : item;
  });
}
